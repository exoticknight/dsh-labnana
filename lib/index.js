...
async function fetchWithTimeout(url, init, timeoutMs, consume, signal) {
  ...
  try {
    ...
    if (timedOut) {
      const timeout = new Error(`labnana: request timed out after ${Math.round(timeoutMs / 1e3)}s`);
...
async function callApi(key, method, apiPath, body, timeoutMs, signal) {
  ...
  let result;
  try {
    result = await fetchWithTimeout(`${BASE_URL}${apiPath}`, init, timeoutMs, async (response2) => {
      let payload2 = null;
      try {
      ...
      return { response: response2, payload: payload2 };
    }, signal);
  } catch (error) {
    if (signal?.aborted) throw signal.reason ?? error;
...
async function keyStateOf(ctx, config) {
  ...
  if (typeof cfg.apiKeyEnv === "string" && cfg.apiKeyEnv.length > 0) {
    ...
    const fromEnv2 = process.env[cfg.apiKeyEnv];
    if (fromEnv2) {
      return { configured: true, source: "env", ref: cfg.apiKeyEnv, masked: maskKey(fromEnv2) };
    }
  }
...
async function downloadImage(dir, url, mimeType, timeoutMs, signal) {
...
async function pollTask(key, taskId, waitSeconds, timeoutMs, signal) {
...
async function subscriptionSummary(key, signal) {
  const data = await callApi(key, "GET", EP_SUBSCRIPTION, null, 2e4, signal);
  if (!data) return {};
  const free = [];
...
var inject = [];
function createConfig(live) {
  const field = (schema) => live ? schema.volatile() : schema;
  return z.object({
...
var LegacySettingsConfig = createConfig(false);
function readConfig(config) {
  ...
  return {
    apiKeyEnv: read(values.apiKeyEnv),
    saveToDisk: read(values.saveToDisk),
    defaultModel: read(values.defaultModel),
    defaultImageSize: read(values.defaultImageSize),
    defaultAspectRatio: read(values.defaultAspectRatio),
    outputDir: read(values.outputDir),
    timeoutSeconds: read(values.timeoutSeconds)
  };
}
function apply(ctx, config) {
  ...
  ctx.inject(["settings"], (sctx) => {
    ...
    sctx.effect(() => sctx.on("settings/document-updated", (namespace) => {
      if (namespace === CONFIG_FORM_NS) refreshPrompt?.();
    }), "dsh-labnana: settings updates");
  });
  async function generateImage(args, cfg, key, workspaceCwd, signal) {
    signal.throwIfAborted();
...
warnings: ${value.warnings.join("; ")}` : ""}`
            ...
            async execute(args, exec) {
              ...
              const cfg = current();
              const payload = await buildPayload(args, cfg);
              const data = await callApi(key, "POST", EP_ESTIMATE, payload, 3e4, exec.signal);
              const out = {
                ok: true,
                model: payload.model,
...
${lines.join("\n")}`
            ...
            async execute(args, exec) {
              ...
              const data = await callApi(key, "GET", `${EP_TASKS}/${taskId}`, null, 3e4, exec.signal);
      ...
      try {
        disposers.push(server.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            if (!body || typeof body !== "object" || Array.isArray(body) || body.apiKey !== void 0 && typeof body.apiKey !== "string") {
              writeJson(res, 400, { ok: false, code: "bad-request", message: "malformed JSON body" });
              return;
            }
            writeJson(res, 200, await testConnection(typeof body.apiKey === "string" ? body.apiKey.trim() : void 0));
          }
        }));
        disposers.push(server.webServer.register({
          kind: "exact",
